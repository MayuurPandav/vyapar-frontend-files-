...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Safely handle empty or non-JSON responses to avoid errors
      ...
      if (res.ok) {
        ...
        const targetView = r === 'superadmin' ? 'super_admin' : 'dashboard';

        setLaunchTarget(targetView);
  ...
  return (
    <div className="login-body-wrapper" style={{
        ...
        <div className="launch-overlay active" style={{ position: 'absolute', inset: 0, background: 'rgba(3, 10, 25, 0.94)', display: 'flex', alignItems: 'center', justifyItems: 'center', justifyContent: 'center', zIndex: 20, opacity: 1, visibility: 'visible' }}>
          <div className="launch-card" style={{ width: 'min(92%, 420px)', padding: '36px 30px', borderRadius: '28px', backgroundColor: 'rgba(12, 20, 42, 0.95)', border: '1px solid rgba(99, 102, 241, 0.16)', boxShadow: '0 36px 90px rgba(0,0,0,0.45)', textAlign: 'center' }}>
            ...
            <p style={{ color: 'rgba(241, 245, 249, 0.78)', fontSize: '0.95rem', lineHeight: 1.8, margin: '14px 0 24px' }}>
              Preparing your secure dashboard. This will only take a moment.
            </p>
      ...
      <div className="login-container" style={{
        ...
        <div className="logo" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '16px', marginBottom: '20px' }}>
          <div className="logo-icon-wrapper" style={{ background: 'linear-gradient(135deg, #3b82f6, #8b5cf6, #ec4899)', width: '44px', height: '44px', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 10px 15px -3px rgba(99, 102, 241, 0.4)' }}>
            <i className="fas fa-chart-pie" style={{ fontSize: '22px', color: '#fff' }}></i>
          </div>
          <span style={{ fontSize: '28px', fontWeight: 700, background: 'linear-gradient(135deg, #3b82f6, #8b5cf6, #ec4899)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', letterSpacing: '-0.5px' }}>
            Vyapar
          </span>
        </div>

        <div style={{ textAlign: 'center', marginBottom: '28px', marginTop: '-12px' }}>
          <p style={{ color: 'rgba(148, 163, 184, 0.85)', fontSize: '13.5px', lineHeight: '1.5', margin: 0 }}>
            Sign in with your Email, Username, or Employee ID to access your dashboard.
          </p>
        </div>

        <form onSubmit={handleSubmit}>
          {isLogin ? (
            // SIGN IN FIELDS (Unified for all roles)
            <div className="form-group" style={{ marginBottom: '20px' }}>
              <label style={{ display: 'block', fontSize: '12px', fontWeight: 500, color: '#94a3b8', marginBottom: '8px', letterSpacing: '0.5px', textTransform: 'uppercase' }}>Email / Username / ID</label>
              <div className="input-wrapper" style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
                <input
                  type="text"
                  className="login-input"
                  placeholder="e.g. admin@vyapar.com or DP-1001"
                  value={username}
                  onChange={(e) => setUsername(e.target.value)}
                  required
                />
                <i className="fas fa-user" style={{ position: 'absolute', left: '14px', color: '#94a3b8' }}></i>
              </div>
            </div>
          ) : (
            // REGISTRATION FIELDS (Merchant Sign Up)
            <>
              <div className="form-group" style={{ marginBottom: '20px' }}>
                <label style={{ display: 'block', fontSize: '12px', fontWeight: 500, color: '#94a3b8', marginBottom: '8px', letterSpacing: '0.5px', textTransform: 'uppercase' }}>Email / Username</label>
                <div className="input-wrapper" style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
                  <input
                    type="text"
                    className="login-input"
                    placeholder="e.g. admin@vyapar.com"
                    value={username}
                    onChange={(e) => setUsername(e.target.value)}
                    required
                  />
                  <i className="fas fa-envelope" style={{ position: 'absolute', left: '14px', color: '#94a3b8' }}></i>
                </div>
              </div>

              <div className="form-group" style={{ marginBottom: '20px' }}>
                <label style={{ display: 'block', fontSize: '12px', fontWeight: 500, color: '#94a3b8', marginBottom: '8px', letterSpacing: '0.5px', textTransform: 'uppercase' }}>Phone Number</label>
                <div className="input-wrapper" style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
                  <input
                    type="tel"
                    className="login-input"
                    ...
                    required
                  />
                  <i className="fas fa-phone" style={{ position: 'absolute', left: '14px', color: '#94a3b8' }}></i>
                </div>
              </div>
            </>
          )}
          ...
          <div className="form-group" style={{ marginBottom: '20px' }}>
            <label style={{ display: 'block', fontSize: '12px', fontWeight: 500, color: '#94a3b8', marginBottom: '8px', letterSpacing: '0.5px', textTransform: 'uppercase' }}>Password</label>
            <div className="input-wrapper" style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
              <input
                type="password"
                className="login-input"
                ...
                required
              />
              <i className="fas fa-lock" style={{ position: 'absolute', left: '14px', color: '#94a3b8' }}></i>
            </div>
          </div>
          ...
          <button type="submit" className="btn" disabled={loading} style={{ width: '100%', padding: '12px', background: 'linear-gradient(135deg, #3b82f6, #8b5cf6, #ec4899)', color: '#fff', border: 'none', borderRadius: '12px', fontSize: '15px', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', marginTop: '28px', boxShadow: '0 10px 15px -3px rgba(99, 102, 241, 0.3)', position: 'relative', overflow: 'hidden' }}>
